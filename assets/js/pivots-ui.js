(function () {
  ...
  function init() {
    $("#pivotGrid").innerHTML = model.dimensions.map(function (dimension) {
      ...
      return '<section class="pivot-card" id="pivot-' + dimension.id + '" aria-labelledby="pivot-title-' + dimension.id + '"><header><h3 id="pivot-title-' + dimension.id + '">' + dimension.title + '</h3><label>Min<input data-pivot-min="' + dimension.id + '" type="number" min="1" max="5000" step="1" value="1" aria-label="Minimum Movies for ' + dimension.title + '"></label>' + (dimension.id === 'years' ? '<label class="pivot-year-basis">Group By<select id="pivotYearBasis" aria-label="Group Years By"><option value="watched">Watched</option><option value="release">Release</option></select></label>' : '') + '<small data-pivot-count role="status"></small></header><div class="pivot-controls">' + ['category', 'count', 'average'].map(function (sort) { return '<button type="button" class="button pivot-sort" data-pivot-sort="' + sort + '" data-pivot-id="' + dimension.id + '"><span class="pivot-sort-top"><span class="pivot-sort-icon" aria-hidden="true">' + App.icons.markup(sort === 'category' ? 'group' : sort === 'count' ? 'pivotCount' : 'pivotAverage') + '</span><span class="pivot-direction" aria-hidden="true"></span></span><span>' + (sort === 'category' ? 'Category' : sort === 'count' ? 'Count' : 'Average') + '</span></button>'; }).join('') + '</div><div class="pivot-table-scroll" tabindex="0" role="region" aria-label="' + dimension.title + ' pivot table"><table><caption class="visually-hidden">' + dimension.title + ' — watched movie counts and average ratings</caption><thead><tr><th scope="col" data-sort-column="category">' + (dimension.id === 'years' ? 'Year' : dimension.id === 'ratings' ? 'Rating' : 'Name') + '</th><th scope="col" data-sort-column="count">Count</th><th scope="col" data-sort-column="average">Average</th></tr></thead><tbody></tbody></table></div></section>';
    }).join('');
