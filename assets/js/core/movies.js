(function () {
  ...
  function normalize(input) {
    ...
    const movie = {
      ...
      watchedDate: date(v.watchedDate), rating: number(v.rating), review: u.cleanText(v.review, 20000)
    };
  ...
  function validate(movie) {
    ...
    if (movie.status === "watched" && (!movie.watchedDate || movie.rating === null || !movie.review.trim())) errors.push("Watched movies need a watched date, rating, and review.");
    return errors;
  }
  ...
  function searchable(movie) { return [movie.title, movie.tmdbId, movie.how, movie.other, movie.notes, movie.review].concat(movie.genres, movie.productionCompanies, movie.directors, movie.actors, movie.collections).join(" ").toLowerCase(); }
  App.movies = { normalize: normalize, normalizeList: normalizeList, validate: validate, fromTmdb: fromTmdb, color: color, searchable: searchable, date: date };
})();
