(function () {
  ...
  function render() {
    ...
    $("#movieList").innerHTML = visible.length ? '<div class="movie-table-scroll" tabindex="0" role="region" aria-label="Movie Spreadsheet"><table class="movie-table"><caption class="visually-hidden">Saved Movies. Wishlist rows are highlighted. Date is the watched date for watched movies and available date for wishlist movies.</caption><thead><tr>' + headers.map(function (column) { const key = column[1] === 'score' ? (filter === 'wishlist' ? 'priority' : 'rating') : column[1], selected = key === sort.key || (key === 'date' && sort.key === 'watched' && filter === 'watched'); return '<th scope="col" class="movie-col-' + column[1] + '" aria-sort="' + (selected ? (sort.direction === 'asc' ? 'ascending' : 'descending') : 'none') + '"><button type="button" class="movie-column-sort" data-movie-column-sort="' + key + '" aria-label="Sort by ' + (column[1] === 'score' ? key : column[0]) + '">' + column[0] + (selected ? App.icons.markup(sort.direction === 'asc' ? 'sortUp' : 'sortDown') : '') + '</button></th>'; }).join('') + '</tr></thead><tbody>' + visible.map(function (movie) {
      ...
      return '<tr class="' + (watched ? 'movie-watched-row' : 'movie-wishlist-row') + '">' + editable(movie, watched ? 'rating' : 'priority', badge(movie), 'movie-col-score') + (filter === 'wishlist' ? '<td class="movie-col-tmdbAverage"><span class="movie-cell">' + (movie.tmdbAverage == null ? '—' : '<span class="movie-score ave-' + model.averageBand(movie.tmdbAverage) + '" title="TMDB average: ' + movie.tmdbAverage.toFixed(1) + ' out of 10" aria-label="TMDB average: ' + movie.tmdbAverage.toFixed(1) + ' out of 10">' + movie.tmdbAverage.toFixed(1) + '</span>') + '</span></td>' : '') + '<th scope="row" class="movie-col-title"><button type="button" class="movie-title-link" data-edit-movie="' + id + '" title="Edit ' + esc(movie.title) + '">' + esc(movie.title) + '</button><span class="visually-hidden">' + (watched ? 'Watched' : 'Wishlist') + '</span></th>' + editable(movie, watched ? 'review' : 'notes', esc((watched ? movie.review : movie.notes) || '—'), 'movie-col-review') + editable(movie, 'how', howMarkup(movie.how), 'movie-col-how') + editable(movie, watched ? 'watchedDate' : 'availableDate', esc((watched ? movie.watchedDate : movie.availableDate) || '—'), 'movie-col-date') + cell(movie.releaseDate, 'movie-col-release') + editable(movie, 'other', esc(movie.other || '—'), 'movie-col-other') + cell(movie.collections.join(', '), 'movie-col-collections') + cell(movie.genres.join(', '), 'movie-col-genres') + cell(movie.actors.join(', '), 'movie-col-actors') + cell(movie.directors.join(', '), 'movie-col-directors') + cell(movie.productionCompanies.join(', '), 'movie-col-companies') + '</tr>';
